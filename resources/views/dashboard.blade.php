<x-app-layout>
    ...
    <x-card-small class="from-amber-600 to-pink-600 justify-between">
        <x-button route="{{ route('time.show', $topLaps[0]->id) }}">
            <h1 class="w-full h-full text-center text-3xl">YOUR BEST TIME</h1>
        </x-button>
        <p class="text-center text-xl">{{ $topLaps[0]->car->brand . ' | ' . $topLaps[0]->car->model ?? '' }}</p>
        <p class="text-center text-2xl font-semibold">{{ $topLaps[0]->lap_time ?? 'No times yet' }}</p>
        <p class="text-center text-xl">{{ $topLaps[0]->circuit->name ?? '' }}</p>
    </x-card-small>

    <x-card-small class="bg-gradient-to-br from-green-500 to-green-900 justify-between">
        <x-button route="{{ route('time.show', $lastLap->id) }}">
            <h1 class="w-full h-full text-center text-3xl">LAST LAP</h1>
        </x-button>

        <p class="text-center text-xl">{{ $lastLap->car->brand . ' | ' . $lastLap->car->model ?? '' }}</p>
        <p class="text-center text-2xl font-semibold">{{ $lastLap->lap_time ?? 'No times yet' }}</p>
        <p class="text-center text-xl">{{ $lastLap->circuit->name ?? '' }}</p>
    </x-card-small>

    <x-card-small class="bg-gradient-to-t from-blue-500 to-blue-900">
        <x-button route="{{ route('car.index') }}">
            <h1 class="w-full h-full text-center text-3xl">YOUR MOST USED CAR</h1>
        </x-button>

        <p class="text-center text-2xl font-semibold">{{ $mostUsedCar->brand . ' | ' . $mostUsedCar->model ?? 'No cars yet' }}</p>
    </x-card-small>

    <x-card-small class="bg-gradient-to-t from-purple-500 to-purple-900">
